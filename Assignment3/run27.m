...
% %n = [0 ,0.1*n_c_max, 0.2*n_c_max, 0.3*n_c_max, 0.4*n_c_max, 0.42*n_c_max 0.5*n_c_max, 0.6*n_c_max, 0.7*n_c_max, 0.8*n_c_max, 0.9*n_c_max, 1*n_c_max];
...
%plot(f, n, u)
%% Sim
sim MSFartoystyringtask27 % The measurements from the simulink model are automatically written to the workspace.
...
for i = 1:length(p(:,1))-1
    chi(i) = atan2(p(i+1,2) - p(i,2),p(i+1,1) - p(i,1));
end
chi(end) = chi(end-1);
chi = mod(chi, 2*pi);
...
plot(t, x_d*rad2deg, '--r');
plot(t, beta*rad2deg, 'k');
legend('\chi', '\psi', '\chi_d', '\beta');
title('Crab angle Compensation')
